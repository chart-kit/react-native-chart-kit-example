...
const healthRingValueKeys: Array<keyof HealthRingPoint & string> = ["progress"];
...
const HealthRangeToggle = ({
  ...
  const highlightX = useRef(
    new Animated.Value(activeIndex * segmentStride),
  ).current;
  ...
  return (
    <View style={[styles.healthRangeToggleDock, { width }]}>{content}</View>
  );
};

const HealthRingMetric = ({ item }: { item: HealthRingPoint }) => (
...
export const HealthActivityAppScreen = ({
        ...
        >
          ...
          <View style={styles.healthRingMetrics}>
            {animatedRings.map((item) => (
              <HealthRingMetric key={item.metric} item={item} />
            ))}
          </View>
        </View>
      </View>
